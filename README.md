...
```
arg-huaian-village/
├── index.html          ← 表层入口：云隐文化研究所首页（访客态，需登录后解锁「研究项目」）
├── about.html          ← 表层：关于我们
├── projects.html       ← 表层→中间层桥接：研究项目（需登录，含日志密码与隐藏入口）
...
1. **index.html** → 页面以"访客"身份呈现，顶部导航默认不显示"研究项目"；查看网页源代码，HTML 注释中藏有管理员登录线索（账号：所内简称 / 密码：反复出现的数字——老项目的生日）；结合 `publications.html` 的 DOI、`contact.html` 的电话/门牌等多处交叉验证，得出账号 `yunyin`、密码 `19870815`，登录后导航解锁「研究项目」
2. **projects.html** → 槐安村项目卡片的"项目编号: HAC-1987-0815"提示格式；数字部分 `19870815` 即日志密码；页面底部隐藏入口**不靠按钮**，而是需**将鼠标悬停在特定批注文字上**才短暂显现，且数秒后自动消失；"沈墨的日志"链接**居中显示**，指向 `journal.html`
...
6. **ledger.html** → 残账册棋盘密码：册背原绘 5×5 方格，每格一字；名册中**朱笔圈定**的数字坐标为真，其余被水渍晕开的数字为干扰。按原序读取坐标（如 `11 12 13 14 15`）在方格中觅字，得 `渡口莫回头`。玩家点击"查看残页注记"获提示后输入验证，正确才显现镜书入口
7. **mirror.html** → 镜中书：残纸经火，仅最上一行字迹可从镜面倒影中辨出，CSS `scaleX(-1)` 水平镜像渲染；其余两行为水渍污痕干扰。将最上一行每个镜像字转正、按原序连读得 `回头即沉`，输入验证通过才显现卦痕入口
8. **omen.html** → 卦痕：朱砂卦象中混有相似卦与朱笔划去的废卦，仅取有效卦；部分卦带"动爻"，需先将该爻阴阳互换得真卦，再查沈墨卦例映射（☰=中 ☷=元 ☳=子 ☴=夜 等），最后自左向右连读有效真卦得 `中元子夜`。玩家点击"查看沈墨的卦例"获提示后输入验证，正确才显现地图室入口
